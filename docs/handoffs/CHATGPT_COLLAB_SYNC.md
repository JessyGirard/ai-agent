...
- Protected regression gate currently: **`499`** scenarios (`python tests/run_regression.py`). Confirm with **`SESSION_SYNC_LOG.md` bottom** (**`→ CHATGPT: READ THIS ENTRY FIRST ←`** — **`DOC-SYNC-03`**, 2026-04-19) if this drifts; **`DOC-SYNC-01`** (391) is historical. **Retrieval + snapshot:** **RETRIEVAL-07–10 + PACKAGING-01** in the same log (search titles). **Read-only package prefaces:** **PACKAGING-02**–**PACKAGING-07** on **`build_project_memory_package`**. **Runtime + reasoning + interaction lane:** **RUNTIME-01** … **RUNTIME-06** + **REASONING-01/02/03/04/05/06** + **INTERACTION-01/01.1/01.2** in **`services/prompt_builder.py`** (reasoning gate, conversation gate, and clarification override). **Memory quality:** **MEMORY-QUALITY-01** … **MEMORY-QUALITY-05** in **`services/memory_service.py`**, including contamination/bleeding grounding control (**MQ-05**). **Memory lane:** **`MEMORY-01` through `MEMORY-10`** — see **`SESSION_SYNC_LOG.md`** and **`docs/specs/memory_log_system.md`**.
...
- Regression gate: unset FETCH_MODE in the shell before python tests/run_regression.py unless a test intentionally sets it. Latest baseline: **499** scenarios — confirm SESSION_SYNC_LOG.md bottom (**DOC-SYNC-03** / **READ THIS ENTRY FIRST**); **DOC-SYNC-01** (391) is historical only. **MEMORY roadmap:** **MEMORY-01 … MEMORY-10** per **`SESSION_SYNC_LOG.md`** and **`memory_log_system.md`**. **Packaging (read-only):** **PACKAGING-02**–**PACKAGING-07** on **`build_project_memory_package`**. **Runtime + reasoning + interaction:** **RUNTIME-01** … **RUNTIME-06** + **REASONING-01/02/03/04/05/06** + **INTERACTION-01/01.1/01.2** on **`build_messages`**. **Memory quality:** **MEMORY-QUALITY-01** … **MEMORY-QUALITY-05** — **`memory_service`** filters with contamination grounding control.
